...
includeLocales:: list of locale tags (BCP 47) to pass to `--include-locales`. +
    The plugin normalizes tags (for example, `EN-us` becomes `en-US`) and removes duplicates. +
...
[purple]##_NOTE_##: If `jlink.options` already contains `--include-locales`,
that explicit command-line option takes precedence over `includeLocales` and `includeLocalesFile`.
[#javaHome]

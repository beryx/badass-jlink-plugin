...
imageOutputDir:: the directory passed as argument to the `--output` option when running `jpackage` in the `create-app-image` mode.
    _defaultValue_: `_buildDir_/_outputDir_` +
    _usage example_: `imageOutputDir = file("$buildDir/my-packaging-image")`

imageName:: the argument passed to the `--name` option when running `jpackage` in the `create-app-image` mode. +
    _defaultValue_: the `name` value configured in the `launcher` block or  `_project.name_` +
    _usage example_: `imageName = "MyApp"`

imageOptions:: list of additional options to be passed to the `jpackage` executable in the `create-app-image` mode. +
    _defaultValue_: empty list +
...
installerOutputDir:: the directory passed as argument to the `--output` option when running `jpackage` in the `create-app-image` mode.
    _defaultValue_: `_buildDir_/_outputDir_` +
...
- create a platform-specific application image in <font color="saddlebrown">imageOutputDir</font> by executing
        <i>jpackage create-app-image</i> with --module-path=<span class="teal">jlinkJarsDir</span>

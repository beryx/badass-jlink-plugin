...
jvmVersion:: The Java version to be used when creating the custom runtime image.
It is passed as argument to the `--release` option of _jlink_. +
    _defaultValue_: null (no `--release` option is used) +
    _usage example_: `jvmVersion = 17`
...
jlink {
    ...
    targetPlatform('linux-x64', '/usr/lib/jvm/jdk_x64_linux_hotspot_17_0_1')
    targetPlatform('linux-s390x', '/usr/lib/jvm/jdk_s390x_linux_hotspot_17_0_1',
                                                               ['--endian', 'big'])
    ...
}
----
...
jlink {
    ...
    targetPlatform("linux-s390x") {
        jdkHome = "/usr/lib/jvm/linux-s390x/jdk-17.0.1"
    ...
    targetPlatform("mac") {
        jdkHome = jdkDownload("https://github.com/AdoptOpenJDK/openjdk17-binaries/releases/download/jdk-17.0.1%2B12/OpenJDK17U-jdk_x64_mac_hotspot_17.0.1_12.tar.gz") {
            downloadDir = "$buildDir/myMac"
            archiveName = "my-mac-jdk"
            archiveExtension = "tar.gz"
            pathToHome = "jdk-17.0.1+12/Contents/Home"
            overwrite = true
        }
...
appVersion:: the version of the additional application. +
    _defaultValue_: null (the version of the main application is used)
...
jpackageHome:: The path to the JDK providing the jpackage tool. +
    _defaultValue_: the first non-empty value from: +
        ...
        pass:[&nbsp;&nbsp;&nbsp;&nbsp;] - the `JAVA_HOME` environment variable +
    _usage example_: `jpackageHome = "/usr/lib/jvm/jdk17"`
...
includeLocales:: list of locale tags (BCP 47) to pass to `--include-locales`. +
    The plugin normalizes tags (for example, `EN-us` becomes `en-US`), removes duplicates, and uses the same locales for `jlink`, `jpackageImage`, and `jpackage`. +
    When this option is set, the plugin also ensures that `jdk.localedata` is added to the modules passed to `jlink`. +
    _defaultValue_: empty list +
    _usage example_: `includeLocales = ['en', 'de-DE']`

includeLocalesFile:: file containing additional locale tags. +
    The file is read as UTF-8. Tags can be separated by commas, new lines, or both. +
    Values from this file are merged with `includeLocales`, normalized, and deduplicated. +
    _defaultValue_: null +
    _usage example_: `includeLocalesFile = file('src/packaging/locales.txt')`

[purple]##_NOTE_##: Locale tags must be valid BCP 47 tags.
The plugin rejects malformed values such as tags containing `_`, leading/trailing `-`, or `--`.
If invalid tags are found, the build fails and reports all invalid values and their source (`includeLocales` or `includeLocalesFile`).

[purple]##_NOTE_##: If `imageOptions`, `installerOptions`, or `jlink.options` already contain `--include-locales`,
those explicit command-line options take precedence over `includeLocales` and `includeLocalesFile`.
...
jlink {
    ...
    jpackage {
        jpackageHome = '/usr/lib/jvm/jdk17'
...
jlink {
    ...
    jpackage {
        ...
        includeLocales = listOf("en", "de-DE")
        includeLocalesFile = file("src/packaging/locales.txt")
        skipInstaller = false
        installerName = "MyApp"
        installerType = "msi"
...
<tr><td class="red">nonModularJarsDir</td><td><code>nonmodjars</code></td></tr>
